...
final class ShopDetailsViewModel {
    ...
    var shopModel : ShopDetailsModel?
...
extension ShopDetailsViewModel: ShopDetailsVMProtocol {
    ...
    var numberOfSections: Int {
      return filterCategories[0].subcategories?.count ?? 0
    }
    ...
    func numberofPro(indexP: Int) -> Int{
        
        return filterCategories[0].subcategories?[indexP].products?.count ?? 0
    }
    ...
    func onViewDidLoad() {
        ...
        repo?.getShopDetails(shopID, params: parameters) { [weak self] result in
            ...
            case .success(let model):
                ...
                self.products = model.categories[0].subcategories?[0].products ?? []
                self.state = .success
                self.ads = model.ads
...
//    }
    ...
    func viewModelForCell(at section: Int , at index: Int) -> Listable {
        return shopModel?.catProducts?[section].products?[index] as! Listable
    }
    ...
    func didSelectProduct(at section: Int, at index : Int) {
        guard let products  = shopModel?.catProducts?[section].products else {return}
