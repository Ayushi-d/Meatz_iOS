...
final class MainFactory: MainFactoryProtocol {
    ...
    func tabBar() -> MainTabBarController {
        ...
        //tabBar.controllers = [home(), orderView(), boxes(), profile(), settings()]
        tabBar.controllers = [home(), orderView(), boxes(), profile()]
        return tabBar
    }
    ...
    func orderView() -> MyOrdersView {
        let offerVC = R.storyboard.main.myOrdersView()!
        let vm = MyOrdersViewModel(MyOrdersRepo(), coordinator, pushedFromOrderState: false)
        offerVC.viewModel = vm
        offerVC.mainViewModel = MainViewViewModel(coordinator)
        return offerVC
    }
